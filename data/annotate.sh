...
if [ -n "${TAXONOMY_ID}" ]; then
	...
	if notExists "${RESULTS}.dbtype"; then
		...
		if notExists "${RESULTS}.dbtype"; then
			#shellcheck disable=SC2086
			"$MMSEQS" easy-search "${TMP_PATH}/clu_rep" "${PROF_TARGET1}" "${TMP_PATH}/prof1_searchDB" "${TMP_PATH}/search_tmp" ${SEARCH_PAR} --greedy-best-hits --alt-ali 3\
				|| fail "first sequence-profile search died"

			preprocessDb "${TMP_PATH}/prof1_searchDB" "${TMP_PATH}/prof1_searchDB_filt"

			#shellcheck disable=SC2086
			"$MMSEQS" easy-search "${TMP_PATH}/clu_rep" "${PROF_TARGET2}" "${TMP_PATH}/prof2_searchDB" "${TMP_PATH}/search_tmp" ${SEARCH_PAR} --greedy-best-hits --alt-ali 3\
...
if notExists "${TMP_PATH}/tmp_join.tsv"; then
	...
	awk -F '\t' -v OFS='\t' '{sub(/\.[^\.]+$/,"",$3)}1' "${TMP_PATH}/prof1_searchDB_nooverlap.tsv" >> "${TMP_PATH}/tmpfile"; mv -f "${TMP_PATH}/tmpfile" "${TMP_PATH}/prof1_searchDB.tsv"
	...
	rm -f "${TMP_PATH}/prof1_searchDB_nooverlap.tsv"
	...
	awk -F '\t' -v OFS='\t' 'BEGIN{OFS=FS="\t"} NR==FNR{clr[$1]=$2; next} { if ($3 in clr) {$3=clr[$3]; print}}' "${TMP_PATH}/mappingFile" "${TMP_PATH}/prof2_searchDB_nooverlap.tsv" | \
	...
	rm -f "${TMP_PATH}/prof2_searchDB_nooverlap.tsv"
